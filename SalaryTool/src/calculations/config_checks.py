...
class ConfigChecks:
    ...
    def monthly_salary_to_daily(self, days):
        ...
        Returns:
            list: Boolean (True jos hyväksytty arvo, False muuten),
            int/float/string (laskelmassa käytettävä arvo)
        """
        ...
        try:
            ...
            try:
                ...
                print("\n\n")
                print(f"Value >>>{days}<<< in config.cfg should be a positive integer or float")
                return [False, days]
    ...
    def information_lack(self, configuration_name):
        ...
        print("Program will terminate, make the required changes in\
        the config.cfg file and then try again.")
        sys.exit()
